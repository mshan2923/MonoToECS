...
#pragma warning restore 0649
    ...
    private void InitSPH()
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            float y = 2 + (i / rowSize) / rowSize * 1.1f;
            ...
            for (int j = 0; j < particles.Length; j++)
            {
                ...
                if (r < parameters[particles[i].parameterID].smoothingRadius)
                {
                    ...
                    forceViscosity += parameters[particles[i].parameterID].particleViscosity
                        * parameters[particles[i].parameterID].particleMass * (particles[j].velocity - particles[i].velocity) / particles[j].density
                        * (45.0f / (Mathf.PI * Mathf.Pow(parameters[particles[i].parameterID].smoothingRadius, 6.0f))) 
    ...
    private void Integrate()
    {
        for (int i = 0; i < particles.Length; i++)
        {
            ...
            {
                particles[i].position += DT * (particles[i].velocity + forceGravity);
            }
        }
    }
        ...
        for (int i = 0; i < particles.Length; i++)
        {
            for (int j = 0; j < colliders.Length; j++)
            {
                ...
                if (Intersect(colliders[j], particles[i].position, parameters[particles[i].parameterID].particleRadius, out penetrationNormal, out penetrationPosition, out penetrationLength))
                {
                    ...
                    particles[i].velocity = Vector3.Reflect(particles[i].velocity, Vector3.up) * (1 - parameters[particles[i].parameterID].particleDrag);
            ...
            if (particles[i].position.y <= parameters[particles[i].parameterID].particleRadius * 0.5f)
            {
                //if (Vector3.Magnitude(particles[i].velocity) < 0.1f) { return; }/ 속도가 8 ~ 38 랜덤으로 평면에 진동하며 있음
