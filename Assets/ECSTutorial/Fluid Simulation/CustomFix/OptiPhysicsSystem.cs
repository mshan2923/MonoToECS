    ...
    protected override void OnUpdate()
    {
        ...
        for (int typeIndex = 1; typeIndex < uniqueTypes.Count; typeIndex++)
        {
            if (SkipFramCount >= SkipFram)// 매번 N초 마다 실행 + 값 초기화는 for 외부에서 / DelayCount > Delay
            {
                ...
                NativeParallelMultiHashMap<int, int> hashMap = new NativeParallelMultiHashMap<int, int>(particleCount, Allocator.TempJob);//NativeMultiHashMap<int, int> hashMap = new NativeMultiHashMap<int, int>(particleCount, Allocator.TempJob);
